...
async function getMachines(id) {
    ...
    const facs = await fms.collection("factories");
    const res = await facs.aggregate([
        {$match: { "prodlines.prodline_id": id }},
        {$project: {
            factory_id: 1,
            prodlines: 1
        }},
...
async function getTeamRecs(id) {
    ...
    const res = await teams.aggregate([
        ...
        {$match: { "machine.anomaly_records.fixed_by_team": id }},
        {$project: {
            record_id: 1,
...
async function getTeamName(id) {
    ...
    const teams = await fms.collection("teams");
    const res = await teams.find({ team_id: id}).project({ team_leader: 1 }).toArray();
    return res;
}
...
async function removeRecs(id) {
    ...
    machs.updateMany(
        { "anomaly_records.record_id": id },
        { $pull: { anomaly_records: { record_id: id } }}
    );

    teams.updateMany(
        { "fixed_records": id },
        { $pull: { fixed_records: id }}
    );
}
...
(async () => {
    console.log(await getAssertions(1381));
})();